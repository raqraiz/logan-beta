// ============================================================================
// SINGLE SOURCE OF TRUTH for cycle day/phase calculation (Deno/edge-runtime copy).
// Canonical logic extracted from chat-ai (the retroactive-date-fix version).
// Mirror of src/lib/cycleCalculations.ts — keep the two files in sync.
// The ONLY intended difference is the PhaseLengths type import (the client
...
export interface CalculateCycleInfoOptions {
  timezone?: string;
  /**
   * Optional "as of" reference date (YYYY-MM-DD string, or a Date on the
   * client). When provided, day/phase is computed for THAT date instead of
   * today — used for retroactive/backdated questions so no date math is
   * ever left to a model.
   */
  asOfDate?: string | Date | null;
  currentPeriodEndDate?: string | null;
  periodPending?: boolean;
  periodStillActive?: boolean;
  phaseLengths?: CyclePhaseLengths | null;
...
export function calculateCycleInfo(
  ...
  if (asOfDate) {
    if (typeof asOfDate === "string" && /^\d{4}-\d{2}-\d{2}$/.test(asOfDate)) {
      ...
      // Date object (client ring passes local Dates) — use its local fields,
      // matching historical ChatCycleCircle behavior.
  ...
  const prefs: CyclePhaseLengths = phaseLengths ?? {};
