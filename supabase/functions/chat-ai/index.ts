...
serve(async (req) => {
  ...
  try {
    ...
    if (postpartumVetoBlockedStageSignal) {
      systemPrompt += `\n\nRUNTIME CONTEXT (this turn only) — POSTPARTUM STAGE-CHANGE VETOED: The user is within 18 months postpartum and mentioned something that could imply a different life stage (cycling, irregular, perimenopause, or menopause). Her profile was intentionally NOT changed.\n\nABSOLUTE RULE for this reply:\n- Do NOT say she "is" in that stage, "is going through" that stage, "is balancing" that stage, or any phrasing that treats it as confirmed fact.\n- Do NOT write anything like "given you're balancing breastfeeding and perimenopause" or "since you're in perimenopause."\n- Reflect it ONLY as a possibility she raised: "possible perimenopause signs," "some overlap with perimenopause symptoms," "it makes sense you're wondering about that while breastfeeding," or "we can keep an eye on whether that pattern continues after your cycle returns."\n- Keep the response focused on what she actually shared (symptom, feeling, or question).\n- Do NOT ask for a last-period date or suggest resetting her cycle.`;
    }
    ...
    if (emotionalContextActive) {
      ...
      } This instruction OVERRIDES the standing guidance to reference her cycle phase and give phase-appropriate tips: for this turn, do NOT open with her cycle day, do NOT offer generic phase lifestyle advice (workouts, meals, macros, "lean into your energy"), and do NOT turn this into a physiology lesson. The PHASE AUTHORITY RULE still applies to any phase you DO mention (never contradict the current phase/day) — but mentioning the phase at all is optional here and should be at most one short line of validation, not the substance of the reply. Give a concrete, practical, human answer to her actual situation.`;
    }
